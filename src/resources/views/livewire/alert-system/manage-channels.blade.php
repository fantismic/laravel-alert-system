<div class="w-full max-w-5xl lg:max-w-full mx-auto mt-2">
                ...
                <tbody>
                    @forelse ($channels as $channel)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 transition border-b border-gray-100 dark:border-gray-800">
                            <td class="p-3 text-gray-900 dark:text-gray-100 font-medium">{{ $channel->name }}</td>
                            <td class="p-3 text-right">
                                <button wire:click="showEdit({{ $channel->id }})"
                                ...
                                <button wire:click="delete({{ $channel->id }})"
                                    ...
                                    onclick="return confirm('¿Eliminar canal?')" title="Eliminar">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    ...
                    @empty
                        <tr>
                            <td colspan="2" class="text-center p-6 text-gray-400 dark:text-gray-500">Sin canales</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="md:hidden flex flex-col gap-4 mt-2">
            ...
            <div class="rounded-xl bg-white/90 dark:bg-gray-900/90 border border-gray-200 dark:border-gray-800 shadow-none px-4 py-3 flex flex-col gap-2">
                <div class="flex items-center justify-between mb-1">
                    ...
                    <div class="flex gap-1">
                        ...
                        <button wire:click="delete({{ $channel->id }})"
                            class="p-2 rounded hover:bg-red-100 dark:hover:bg-red-900/20 transition" onclick="return confirm('¿Eliminar canal?')" title="Eliminar">
                            <svg class="h-5 w-5 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                <path d="M6 18L18 6M6 6l12 12" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                        </button>
        ...
        @empty
            <div class="text-gray-400 dark:text-gray-500 text-center">Sin canales</div>
        @endforelse
        <div class="pt-4">
            {{ $channels->links('pagination::tailwind') }}
        </div>
    ...
    <div class="fixed bottom-8 right-8 z-50 md:hidden">
        <button
            type="button"
            wire:click="showCreate"
            class="flex items-center justify-center w-14 h-14 rounded-full bg-gray-200 hover:bg-gray-300 text-gray-800 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-gray-100 shadow-lg text-2xl transition"
            title="Nuevo canal"
        >
            <svg class="w-7 h-7 text-gray-600 dark:text-gray-200" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/>
            </svg>
        </button>
    </div>
